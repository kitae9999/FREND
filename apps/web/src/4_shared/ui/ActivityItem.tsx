...
const Badge = ({ text }: { text: string }) => {
  return (
    <span className="inline-flex items-center rounded-full bg-[#ED1B25] px-2 py-1 text-[10px] font-extrabold tracking-wide text-white">
      {text}
    </span>
  );
};
...
export const ActivityItem = ({
  ...
  return (
    <article
      className={[
        "group relative overflow-hidden rounded-[18px] border border-solid border-[#2A2A3E] shadow-sm transition",
    ...
    >
      ...
      <div className="relative flex h-full flex-col p-6">
        {kind === "club" && (
          <div className="mb-4 flex items-start gap-3">
            ...
            <div className="ml-auto">
              <button className="rounded-full bg-[#ED1B25] px-3 py-1.5 text-xs font-extrabold text-white hover:bg-[#ff2b34]">
                Join
              </button>
            </div>
          </div>
        ...
        {kind === "post" && (
          <>
            <div className="mb-4">
              <p className="text-sm font-extrabold text-white">{author ?? "Someone"}</p>
              <p className="mt-1 text-xs text-white/50">{time ?? "just now"}</p>
            </div>
        ...
        {kind === "stat" && (
          <div className="flex h-full flex-col items-center justify-center text-center">
            ...
            <div className="mt-2 text-3xl font-[950] text-white">A+</div>
            <div className="mt-3 text-sm font-extrabold text-white/90">{title}</div>
